...
class AudioplayersWindowsPlugin : public Plugin {
 ...
 private:
  ...
  static inline PluginRegistrarWindows* pluginRegistrar = nullptr;
  static inline DWORD platformThreadId = 0;
  static inline std::queue<std::function<void()>> eventQueue{};
  static inline std::mutex eventQueueMutex{};
  static inline const UINT kDispatchEventMessage = WM_APP + 0x3A77;
  static inline int windowProcDelegateId = 0;
  ...
  static void DispatchOnPlatformThread(std::function<void()> task);
...
void AudioplayersWindowsPlugin::RegisterWithRegistrar(
  ...
  globalEvents->SetDispatcher([](std::function<void()> task) {
    AudioplayersWindowsPlugin::DispatchOnPlatformThread(std::move(task));
  });
  ...
  windowProcDelegateId = registrar->RegisterTopLevelWindowProcDelegate(
      [](HWND hwnd, UINT message, WPARAM wparam, LPARAM lparam)
        ...
        if (message == kDispatchEventMessage) {
          AudioplayersWindowsPlugin::DrainEventQueue();
          return 0;
        }
        return std::nullopt;
      });
...
void AudioplayersWindowsPlugin::CreatePlayer(std::string playerId) {
  ...
  eventHandler->SetDispatcher([](std::function<void()> task) {
    AudioplayersWindowsPlugin::DispatchOnPlatformThread(std::move(task));
  });
  auto _obj_stm_handle =
...
void AudioplayersWindowsPlugin::DispatchOnPlatformThread(
    std::function<void()> task) {
  if (GetCurrentThreadId() == platformThreadId) {
    task();
    return;
  }
  {
  ...
  if (pluginRegistrar && pluginRegistrar->GetView()) {
    HWND hwnd = pluginRegistrar->GetView()->GetNativeWindow();
    if (hwnd) {
      PostMessage(hwnd, kDispatchEventMessage, 0, 0);
    }
  }
}

void AudioplayersWindowsPlugin::DrainEventQueue() {
