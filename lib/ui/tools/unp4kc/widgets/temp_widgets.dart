...
class ImageTempWidget extends HookWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (!file.existsSync()) {
      return Center(child: Text(S.current.tools_unp4k_msg_unknown_file_type(filePath), textAlign: TextAlign.center));
    }
      ...
      child: Center(
        child: Image.file(
          ...
          errorBuilder: (context, error, stackTrace) {
            return Text(S.current.tools_unp4k_msg_unknown_file_type(filePath), textAlign: TextAlign.center);
          },
        ),
      ),
    );
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    Future<void> renderModel() async {
      ...
      try {
        ...
        if (result.success && result.rgbaData != null) {
          ...
          image.value = await completer.future;
    ...
    Future<void> initializeSession() async {
      try {
        ...
        final result = await model_api.p4KModelSessionCreate(
          glbPath: glbPath,
          width: width,
          height: height,
          bgColor: bgColor,
    ...
    useEffect(() {
      if (!File(glbPath).existsSync()) {
        errorMessage.value = "GLB file not found: $glbPath";
        isInitializing.value = false;
        return null;
      }
      initializeSession();
